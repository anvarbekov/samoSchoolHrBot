...
export async function handleUpdate(update) {
  ...
  if (message.text === '/start') {
    ...
    await sendMessage(
      ...
      `Ushbu bot orqali siz bizning maktabimizga ishga ariza topshirishingiz mumkin.\n\n` +
      `📋 Bir necha savollarga javob bering va biz siz bilan bog'lanamiz!\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `<b>1-qadam:</b> Ism va Familiyangizni kiriting:\n` +
      `<i>Masalan: Abdullayev Jasur</i>`,
  ...
  if (!session) {
    await sendMessage(chatId, `👋 Salom! Ishga ariza topshirish uchun /start buyrug'ini yuboring.`)
    return
  }
  ...
  if (step === STEPS.FULL_NAME) {
    ...
    await sendMessage(
      ...
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `<b>2-qadam:</b> Mutaxassisligingizni kiriting:\n` +
      `<i>Masalan: O'qituvchi, Metodist, Logoped</i>`
    )
    return
  }
  ...
  if (step === STEPS.SPECIALTY) {
    ...
    await setSession(chatId, { step: STEPS.EXPERIENCE, data: { ...data, specialty: message.text.trim() } })
    await sendMessage(
      chatId,
      `✅ <b>Mutaxassislik:</b> ${message.text.trim()}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `<b>3-qadam:</b> Ish stajingizni tanlang:`,
      {
        reply_markup: {
          keyboard: [
            [{ text: "🆕 Tajribam yo'q" }, { text: '📚 1 yilgacha' }],
  ...
  if (step === STEPS.EXPERIENCE) {
    ...
    await setSession(chatId, { step: STEPS.CURRENT_WORK, data: { ...data, experience: message.text.trim() } })
    await sendMessage(
      chatId,
      `✅ <b>Ish staji:</b> ${message.text.trim()}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `<b>4-qadam:</b> Hozirda qayerda ishlayapsiz?\n` +
      `<i>Agar ishlamayotgan bo'lsangiz "Ishsizman" deb yozing</i>`,
      { reply_markup: { remove_keyboard: true } }
  ...
  if (step === STEPS.CURRENT_WORK) {
    ...
    await setSession(chatId, { step: STEPS.TARGET_POSITION, data: { ...data, currentWork: message.text.trim() } })
    await sendMessage(
      chatId,
      `✅ <b>Hozirgi ish joyi:</b> ${message.text.trim()}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `<b>5-qadam:</b> Maktabimizga qaysi yo'nalish bo'yicha ishga kirmoqchisiz?\n` +
      `<i>Quyidagi tugmalardan birini tanlang:</i>`,
      {
        reply_markup: {
          keyboard: [
            [{ text: "📚 Boshlang'ich sinf o'qituvchisi (rus sinflari uchun)" }],
            [{ text: "📚 Boshlang'ich sinf o'qituvchisi (ingliz sinflari uchun)" }],
  ...
  if (step === STEPS.TARGET_POSITION) {
    if (!message.text || message.text.length < 2) {
      await sendMessage(chatId, `⚠️ Iltimos, yo'nalishni tanlang.`)
      return
    }
    await setSession(chatId, { step: STEPS.REGION, data: { ...data, targetPosition: message.text.trim() } })
    await sendMessage(
      chatId,
      `✅ <b>Istalgan yo'nalish:</b> ${message.text.trim()}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `<b>6-qadam:</b> Qaysi viloyat, tuman/shahar va MFYdan ekansiz?\n` +
      `<i>Masalan: Andijon viloyati, Xo'jaobod tumani, Do'stlik MFY</i>`,
      {
        reply_markup: {
          keyboard: [
            [{ text: '🏙️ Toshkent shahri' }, { text: '🌆 Toshkent viloyati' }],
  ...
  if (step === STEPS.REGION) {
    ...
    await setSession(chatId, { step: STEPS.PHONE, data: { ...data, region: message.text.trim() } })
    await sendMessage(
      chatId,
      `✅ <b>Hudud:</b> ${message.text.trim()}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `📱 <b>7-qadam:</b> Telefon raqamingizni yuboring:\n\n` +
      `<i>Tugma orqali avtomatik yuborishingiz yoki qo'lda yozishingiz mumkin</i>\n` +
  ...
  if (step === STEPS.PHONE) {
    ...
    await setSession(chatId, { step: STEPS.PHOTO, data: { ...data, phone } })
    await sendMessage(
      chatId,
      `✅ <b>Telefon:</b> ${phone}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `📸 <b>8-qadam:</b> O'z rasmingizni yuboring:\n\n` +
      `<i>⚠️ Iltimos, rasmiy va aniq rasm yuboring (passport yoki biznes rasmingiz)</i>`,
      {
        reply_markup: {
          keyboard: [[{ text: "⏭️ O'tkazib yuborish" }]],
  ...
  if (step === STEPS.PHOTO) {
    ...
    if (message.text === "⏭️ O'tkazib yuborish") {
      ...
      await sendMessage(
        chatId,
        `⚠️ Iltimos, rasm yuboring yoki "O'tkazib yuborish" tugmasini bosing.`,
        {
          reply_markup: {
            keyboard: [[{ text: "⏭️ O'tkazib yuborish" }]],
            resize_keyboard: true,
    ...
    await setSession(chatId, { step: STEPS.CV, data: { ...data, photoUrl } })
    await sendMessage(
      chatId,
      `${photoUrl ? '✅ Rasm muvaffaqiyatli yuklandi!\n\n' : "⏭️ Rasm o'tkazib yuborildi.\n\n"}` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `📄 <b>9-qadam (so'nggi):</b> CV yoki rezyumengizni yuboring:\n\n` +
      `<i>PDF, Word yoki rasm formatida yuborishingiz mumkin</i>`,
      {
        reply_markup: {
          keyboard: [[{ text: "⏭️ CV yo'q, o'tkazib yuborish" }]],
  ...
  if (step === STEPS.CV) {
    ...
    await sendMessage(
      ...
      `💼 <b>Istalgan yo'nalish:</b> ${finalData.targetPosition}\n` +
      `📍 <b>Hudud:</b> ${finalData.region}\n` +
      `📱 <b>Telefon:</b> ${finalData.phone || '—'}\n` +
      `🔗 <b>Telegram:</b> ${finalData.username ? '@' + finalData.username : '—'}\n` +
      `🖼️ <b>Rasm:</b> ${finalData.photoUrl ? '✅ Yuklandi' : '❌ Yuklanmadi'}\n` +
      `📄 <b>CV:</b> ${finalData.cvUrl ? '✅ Yuklandi' : '❌ Yuklanmadi'}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `🆔 <b>Ariza raqami:</b> <code>${candidate.id.slice(0, 8).toUpperCase()}</code>\n\n` +
      `⏳ Tez orada maktabimiz HR mutaxassislari siz bilan bog'lanishadi!`,
      { reply_markup: { remove_keyboard: true } }
    )
